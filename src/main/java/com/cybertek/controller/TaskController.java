...
public class TaskController {
    ...
    @Operation(summary = "Read all non complete tasks")
    @PreAuthorize("hasAuthority('Employee')")
    public ResponseEntity<ResponseWrapper> employeeReadAllCompleteTask() {
        List<TaskDTO> tasks = taskService.listAllTasksByStatus(Status.COMPLETE);
        return ResponseEntity.ok(new ResponseWrapper("Successfully read completed current user tasks",tasks));
    }
